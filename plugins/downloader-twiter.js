...
let handler = async (m, { usedPrefix, command, conn, text }) => {
  if (!text)
    throw `Gunakan format: ${usedPrefix}${command} https://twitter.com/gofoodindonesia/status/1229369819511709697`;
  m.reply(wait);
  let url = text.replace("x.com", "twitter.com");
  let res = await getTwitterMedia(url);
  let hasil = res.media.map((x) => x.url);
  for (let i of hasil) {
    conn.sendFile(
      m.chat,
      i,
      null,
      `*T W I T T E R      D O W N L O A D E R*\n\n*Tanggal: ${res.date}*\n*Likes: ${res.likes}*\n*Replies: ${res.replies}*\n*ReTweets: ${res.retweets}*`,
      m,
    );
  }
};
...
handler.command = /^twitter|twit|twiter$/i;
