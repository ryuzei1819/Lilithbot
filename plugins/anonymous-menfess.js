let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!text) return conn.reply(m.chat, `[❗] *Penggunaan:* .menfess +${nomorown}|<nama>|<pesan>`, m)
    let [jid, name, pesan] = text.split('|');
    if ((!jid || !name || !pesan)) throw `[❗] *Penggunaan*: .menfess ${nomorown}|<nama>|<pesan>`;
...
quoted: fcon
        ...
        .then(() => {
            conn.reply(m.chat, 'Berhasil mengirim pesan menfess.', m)
            conn.menfess[id] = {
                id,
                dari: m.sender,
                nama: name,
