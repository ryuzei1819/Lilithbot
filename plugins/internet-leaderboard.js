...
5. 💎 *lb diamond* - Leaderboard berdasarkan diamond
...
${sortedDiamond.slice(0, len).map(({ jid, diamond }, i) => {
  mentionedJid.push(jid);
  return `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : "@"}${jid.split`@`[0]} *${diamond} Diamond* 💎`;
}).join`\n`}
`.trim();
      break;

    default:
      text = '⚠️ Kategori leaderboard tidak valid. Kategori yang tersedia: exp, limit, level, money, diamond ⚠️';
      break;
  }
